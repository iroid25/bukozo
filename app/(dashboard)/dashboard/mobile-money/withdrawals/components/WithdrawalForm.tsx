...
export default function MobileMoneyWithdrawalCreateForm({
  isOpen,
  onClose,
  currentUserId,
}: {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<MobileMoneyWithdrawalCreateDTO>();
  ...
  const accountOptions: Option[] = memberAccounts
    .filter((account) => !(account.accountType as any).isShareAccount)
    .map((account) => ({
  ...
  const isWithdrawalValid = () => {
    ...
    const totalDeduction = amount + getMMFee();

    if (!account || amount <= 0) return true;

    return (
      account.balance >= totalDeduction &&
      account.balance - totalDeduction >= account.accountType.minBalance
    );
  };


  const getWithdrawalWarning = () => {
    const account = getSelectedAccount();
    const amount = Number(watchedValues.amount) || 0;
    const totalDeduction = amount + getMMFee();
    ...
    if (account.balance < totalDeduction) {
      return `Insufficient funds. Total deduction (with fees) required: ${formatCurrency(totalDeduction)}. Available: ${formatCurrency(account.balance)}`;
    }

    if (account.balance - totalDeduction < account.accountType.minBalance) {
      return `Withdrawal would leave account below minimum balance of ${formatCurrency(account.accountType.minBalance)}`;
    }
  ...
  const getMMFee = () => {
    ...
    return calculateMobileMoneyFee(amount) + MOBILE_MONEY_SERVICE_FEE;
  };


  async function saveMobileMoneyWithdrawal(
    ...
    try {
      ...
      const response = await fetch("/api/v1/mobile-money/withdrawals", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || result.error) {
        toast.error("Failed to Process Mobile Money Withdrawal", {
      ...
      setLoading(false);
      toast.success("Mobile Money Withdrawal Processed Successfully!", {
        description: `${formatCurrency(formData.amount)} withdrawn. Fee: ${formatCurrency(result.fees?.transferFee + result.fees?.serviceFee)}. Total: ${formatCurrency(result.fees?.totalDeduction)}.`,
      });
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleReset()}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Process a mobile money withdrawal transaction for a SACCO member.
            All fields are required.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit(saveMobileMoneyWithdrawal)}>
          <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            {formatCurrency(
                              getSelectedAccount()!.accountType.minBalance
                            )}
                          </p>
                        </div>
                      </div>
            ...
            {selectedAccount && (
              <div className="space-y-4">
                ...
                {/* Withdrawal Warning */}
                {watchedValues.amount && getWithdrawalWarning() && (
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="description"
                    placeholder="Optional description for this mobile money withdrawal..."
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-red-600">
                            {formatCurrency(getMMFee())}
                          </span>
                        </div>
                        <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
                          <span className="text-gray-600 font-bold">Total Deduction:</span>
                          <span className="font-bold text-red-600">
                            -{formatCurrency((Number(watchedValues.amount) || 0) + getMMFee())}
                          </span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
                          <span className="text-gray-600 font-bold">New Balance:</span>
                          <span className="font-bold text-blue-600">
                            {formatCurrency(
                              (getSelectedAccount()?.balance || 0) -
                                ((Number(watchedValues.amount) || 0) + getMMFee())
                            )}
                          </span>
                        </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t">
              ...
              <SubmitButton
                title="Process Mobile Money Withdrawal"
                loading={loading}

...
interface AccountantEditClientProps {
  user: any;
  branches: any[];
}
...
export default function AccountantEditClient({ user, branches }: AccountantEditClientProps) {
    ...
    try {
      ...
      try {
        ...
        if (response.data.error) {
          toast.error("Failed to save image", { description: response.data.error });
          return;
        }
        ...
        toast.error("Failed to save image", { 
          description: saveError.response?.data?.error || "The image was uploaded but couldn't be saved to your profile." 
        });
      }
    } catch (error) {
  ...
  const onSubmit = async (formData: UpdateUserFormData) => {
    ...
    try {
      ...
      const response = await axios.put("/api/v1/users", updateData);

      if (response.data.error) {
        toast.error("Update Failed", { description: response.data.error });
        return;
      }

      toast.success("Success!", {
        description: "Accountant profile updated successfully",
      });

      setTimeout(() => {
        router.push(`/dashboard/users/accountants`);
        router.refresh();
      }, 1000);
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-12">
      <div className="h-48 w-full bg-gradient-to-r from-blue-800 via-indigo-700 to-indigo-900 relative overflow-hidden">
          ...
          <div className="flex-1 space-y-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 mt-1">
                    Manage accountant permissions and personal information
                  </p>
                </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100 group">
                <div className="p-1 bg-gradient-to-r from-blue-500 to-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                <div className="p-8">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-10">
                    ...
                    <div className="flex-1 space-y-2 text-center sm:text-left">
                      ...
                      <p className="text-slate-500 text-sm leading-relaxed max-w-xs">
                        Upload a clear portrait for system identification.
                      </p>
              ...
              <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100">
                <div className="p-8">
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <h2 className="text-xl font-bold text-slate-800">Work & Permissions</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    {isAdmin && (
                      <div className="md:col-span-2 bg-blue-50/50 border border-blue-100 p-6 rounded-2xl mb-2">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <Label className="text-blue-900 font-bold uppercase tracking-wider text-xs">Role Management</Label>
                        </div>
                        <select
                          {...register("role")}
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <Label className="text-sm font-bold text-slate-700 ml-1 mb-1.5 block">Assigned Branch</Label>
                      <FormSelectInput
                        label=""
                    ...
                    <div className="md:col-span-2 space-y-2 mt-2">
                      <Label className="text-sm font-bold text-slate-700 ml-1 mb-1.5 block">Physical Address</Label>
                      <Textarea
                        {...register("address")}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className={`text-sm ${watched.isActive ? 'text-slate-500' : 'text-rose-600 font-medium'}`}>
                          {watched.isActive 
                            ? "This accountant is currently authorized to access system dashboards."
            ...
            <div className="sticky top-8 space-y-6">
                ...
                <div className="space-y-6 relative z-10">
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-50">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className={`h-2.5 w-2.5 rounded-full ${user.isActive ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-600 rounded-3xl p-6 text-white shadow-xl shadow-blue-700/20 group cursor-default">
                  ...
                  <p className="text-sm font-medium leading-relaxed text-blue-50">
                    All modifications to accountant profiles are logged in the system audit trail.
                  </p>
              </div>
            </div>
          </div>

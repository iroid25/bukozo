...
export default function InstitutionsPage() {
    ...
    try {
      ...
      if (instResp.data.success && branchesResp.data.success) {
        ...
        if (userRole !== "ADMIN" && userBranchId) {
          fetchedInstitutions = fetchedInstitutions.filter(
            (inst: any) => inst.user?.branchId === userBranchId,
          );
        }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6 text-center">
        ...
        <div>
          ...
          <p className="text-neutral-500 mt-2 max-w-[full] bg-fuchsia-600 mx-auto">
            {error}
          </p>
        </div>
        <Button
  ...
  const userBranch = branches.find(
    (b: any) => b.id === session?.user?.branchId,
  );
  ...
  if (userRole === "ADMIN") {
    subtitle =
      "Register, review, approve and manage all institutions across all branches";
  } else {

...
import type { AccountTypeCreateDTO } from "@/types/accountTypes";
...
export default function AccountTypeCreateForm({
  initialData,
  editingId,
  isOpen,
  onClose,
}: {
  initialData?: Partial<AccountTypeCreateDTO> & { isShareAccount?: boolean };
  editingId?: string;
  isOpen: boolean;
  onClose: () => void;
}) {
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<AccountTypeCreateDTO>({
    defaultValues: {
      name: initialData?.name ?? "",
      interestRate: initialData?.interestRate ?? 0,
      minBalance: initialData?.minBalance ?? 0,
      monthlyCharge: initialData?.monthlyCharge ?? null,
      flatWithdrawalFee: initialData?.flatWithdrawalFee ?? null,
      withdrawalFrequencyDays: initialData?.withdrawalFrequencyDays ?? null,
      maxWithdrawal: initialData?.maxWithdrawal ?? null,
      isLoanEligible: initialData?.isLoanEligible ?? true,
      canWithdraw: initialData?.canWithdraw ?? true,
      isShareAccount: initialData?.isShareAccount ?? false,
      isDefault: initialData?.isDefault ?? false,
      earnsDividends: (initialData as any)?.earnsDividends ?? false,
      sharePrice: (initialData as any)?.sharePrice ?? null,
      hasFixedPeriod: initialData?.hasFixedPeriod ?? false,
      fixedPeriodMonths: initialData?.fixedPeriodMonths ?? null,
      maturityTransferAccountType:
        initialData?.maturityTransferAccountType ?? "",
      ledgerAccountId: initialData?.ledgerAccountId ?? null,
    },
  });

  const [loading, setLoading] = useState(false);
  const [interestPeriod, setInterestPeriod] = useState<"MONTHLY" | "ANNUALLY">(
    (initialData as any)?.interestPeriod ?? "ANNUALLY"
  );
  ...
  const watchedIsShareAccount = watch("isShareAccount") ?? initialData?.isShareAccount ?? false;
  ...
  React.useEffect(() => {
    ...
    if (isOpen) void fetchAccounts();
  }, [isOpen, watchedIsShareAccount]);

  async function saveAccountType(data: AccountTypeCreateDTO) {
    try {
      setLoading(true);
      ...
      if (isNaN(interestRate) || isNaN(minBalance)) {
        toast.error("Invalid number format for rates or balances");
        return;
      }
      ...
      const payload = {
        ...
        flatWithdrawalFee: data.flatWithdrawalFee
          ? Number(data.flatWithdrawalFee)
          : null,
        withdrawalFrequencyDays: data.withdrawalFrequencyDays
          ? Number(data.withdrawalFrequencyDays)
          : null,
        maxWithdrawal: data.maxWithdrawal ? Number(data.maxWithdrawal) : null,
        fixedPeriodMonths: data.fixedPeriodMonths
          ? Number(data.fixedPeriodMonths)
          : null,
        maturityTransferAccountType:
          (data.maturityTransferAccountType as string)?.trim() || null,
        sharePrice: data.sharePrice ? Number(data.sharePrice) : null,
        earnsDividends: data.earnsDividends ?? false,
      };

      const url = editingId
        ? `/api/v1/account-types/${editingId}`
      ...
      toast.success(
        editingId ? "Account type updated" : "Account type created",
        {
          description: editingId
            ? "Changes saved successfully."
            : "Redirecting to account type details…",
        }
      );
      ...
      if (!editingId && apiData?.data) {
        setTimeout(() => {
          router.push(`/dashboard/account-types/${apiData.data.id}`);
        }, 800);
      } else {
        router.refresh();
      }
  ...
  const ToggleRow = ({
    ...
    onChange,
    icon: Icon,
    trueColor = "text-emerald-600",
    falseColor = "text-rose-600",
  }: {
    id: string;
    label: string;
    description?: string;
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure savings product conditions. Fields marked * are required.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit(saveAccountType)} className="space-y-6 pt-2">
          {/* ── 1. Basic Info ── */}
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <TextInput
                ...
                name="name"
                icon={Wallet}
                placeholder="e.g., Voluntary Savings, Junior Savings"
                validation={{
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  How often interest is compounded / paid.
                </p>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground bg-muted/50 rounded-lg px-3 py-2">
                Tiered withdrawal fees (e.g., UGX 300 / 500 / 700 / 1,000 by amount) are configured separately on the <strong>Fees</strong> tab after creating the account type.
              </p>
            </div>
          </div>

          {/* ── 4. Fixed Savings / Lock Period ── */}
          <div>
            ...
            <div className="space-y-3">
              <ToggleRow
                ...
                description="Funds are locked for a set period; early withdrawal forfeits interest."
          ...
          <div>
            ...
            <ToggleRow
              id="isLoanEligible"
              label="Loan Eligible"
              description="Balance in this account type is counted when assessing loan eligibility."
          ...
          <div>
            ...
            <div className="space-y-2">
              <Label>Map to Existing GL Account (optional)</Label>
              <Select
                value={watch("ledgerAccountId") || undefined}
                onValueChange={(val) => setValue("ledgerAccountId", val)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Leave blank — system will auto-generate a control account" />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-xs text-muted-foreground">
                If left blank, a new control account is auto-generated under{" "}
                {watchedIsShareAccount ? "Equity (3xxxx)" : "Liabilities (2xxxx)"}.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-muted/30 px-4 py-3 text-xs space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-1">
              ...
              <span
                className={
                  watchedCanWithdraw ? "text-emerald-600 font-medium" : "text-rose-600 font-medium"
                }
              >
              ...
              {watchedHasFixedPeriod && (
                <>
                  ...
                  <span className="font-medium">
                    {watchedFixedPeriodMonths
                      ? `${watchedFixedPeriodMonths} months`
                      : "Not set"}
                  </span>
                </>
              )}
              ...
              {watch("withdrawalFrequencyDays") && (
                <>
                  ...
                  <span className="font-medium">
                    Every {watch("withdrawalFrequencyDays")} days
                  </span>
                </>
              )}
          ...
          <div className="flex items-center justify-between pt-2 border-t">
            ...
            <SubmitButton
              title={editingId ? "Update Account Type" : "Create Account Type"}
              loading={loading}
            />
          </div>
        </form>

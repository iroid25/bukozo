...
import { buildIncomeExpenseReport, buildIncomeExpenseWorkbookRows } from "@/lib/reports/income-expense-report";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = buildIncomeExpenseWorkbookRows(report);
    ...
    return new NextResponse(buffer as unknown as BodyInit, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="statement-of-comprehensive-income-and-expenditure-${new Date().toISOString().slice(0, 10)}.xlsx"`,
      },
    });
  } catch (error) {
    console.error("Income & Expenses export error:", error);

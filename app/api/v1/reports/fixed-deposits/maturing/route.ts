...
import { getUpcomingMaturingFixedDepositsReport } from "@/lib/reports/fixed-deposits-report";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getUpcomingMaturingFixedDepositsReport({
      user,
    ...
    console.error("Error generating fixed deposit maturing report:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to generate fixed deposit maturing report",

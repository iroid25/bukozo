...
import { buildFixedDepositWithdrawnWorkbook, getFixedDepositWithdrawnReport } from "@/lib/reports/fixed-deposits-report";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getFixedDepositWithdrawnReport({
      user,
    ...
    const buffer = await buildFixedDepositWithdrawnWorkbook(report);
    return new NextResponse(Buffer.from(buffer), {
      status: 200,
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="fixed-deposits-withdrawn-${report.dateRange.to}.xlsx"`,
      },
    });
  } catch (error) {
    console.error("Error exporting fixed deposit withdrawn report:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to export fixed deposit withdrawn report",

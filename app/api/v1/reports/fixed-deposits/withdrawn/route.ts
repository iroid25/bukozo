...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getFixedDepositWithdrawnReport({
      user,
    ...
    console.error("Error generating fixed deposit withdrawn report:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to generate fixed deposit withdrawn report",

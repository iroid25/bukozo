...
export async function GET(request: NextRequest) {
  try {
    ...
    const threshold = searchParams.get("threshold") ? Number(searchParams.get("threshold")) : 1_000_000;
    ...
    return NextResponse.json({
      ...
      data: {
        ...report,
        transactions: report.transactions.filter((row) => row.largeTransactionFlag),
      },
    });
  } catch (error) {
    console.error("Error generating large savings transactions report:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to generate large savings transactions report",

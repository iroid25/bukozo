  ...
  try {
    ...
    return new NextResponse(Buffer.from(buffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="cash-flow-review-balance-sheet-${report.generatedDate.replaceAll("/", "-")}.xlsx"`,
      },
    });
  } catch (error) {
    console.error("Cash flow review balance sheet export error:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to export cash flow review balance sheet",

...
// POST /api/v1/journal-entries/auto/loan-disbursement - Create journal entry for loan disbursement
export async function POST(request: NextRequest) {
  try {
    ...
    // Find the loan account (1 - ASSETS - Loans)
    ...
    // Create journal entries (Debit Loan Receivable, Credit Cash)
    ...
    return NextResponse.json(
      {
        ...
        message: "Loan disbursement journal entry created successfully",
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("Error creating loan disbursement journal entry:", error);
    return NextResponse.json(

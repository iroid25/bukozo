...
async function main() {
  console.log("🚀 Starting Asset-COA Linkage Migration...");
  ...
  for (const asset of assets) {
    try {
      ...
      let coaAccount = await db.chartOfAccount.findFirst({
        where: {
          OR: [
      ...
      if (coaAccount) {
        ...
        if (asset.category.toUpperCase().includes("FURNITURE")) parentCode = "103100";
        else if (asset.category.toUpperCase().includes("COMPUTER")) parentCode = "103100";
        else if (asset.category.toUpperCase().includes("VEHICLE") || asset.category.toUpperCase().includes("MOTOR")) parentCode = "103200";
        else if (asset.category.toUpperCase().includes("BUILDING") || asset.category.toUpperCase().includes("LAND")) parentCode = "103000";

        const parentAccount = await db.chartOfAccount.findUnique({ where: { accountCode: parentCode } });
        if (!parentAccount) {
        ...
        // Generate Code
        const latestChild = await db.chartOfAccount.findFirst({
        ...
        if (latestChild) {
          ...
          if (suffix) nextNum = parseInt(suffix) + 1;
        }
        const newCode = `${parentCode}${nextNum.toString().padStart(2, '0')}`;

        coaAccount = await db.chartOfAccount.create({
          data: {
      ...
      const depExpAccount = await db.chartOfAccount.findFirst({ where: { accountCode: depExpCode } });
      ...
      console.error(`  ❌ Error processing asset ${asset.assetName}:`, error.message);
    }
  }
